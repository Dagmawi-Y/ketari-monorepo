...
const SearchComponent = () => {
  ...
  let categoryContent;
  if (isLoading) {
    categoryContent = <option className="text-[#8c6432]">Loading</option>;
  } else if (error) {
    categoryContent = (
      <option className="text-[#8c6432]">Something went wrong</option>
    );
  } else if (data?.all_category?.length === 0) {
    categoryContent = (
      <option className="text-[#8c6432]">There is no Category</option>
    );
  } else if (data?.all_category?.length > 0) {
  ...
  return (
    <section>
      <div className="container mx-auto">
        <div className="shadow-lg z-10 p-5 border bg-[#fff]">
          <form
          ...
          >
            ...
            <select
              required
              defaultValue=""
              className="select select-bordered w-full rounded-none text-[#fec557]"
              {...register("location")}
            >
